...
const AI_VERDICTS_URL = 'https://raw.githubusercontent.com/ArikGarbuz/DailyStockMagazine/main/ai-verdicts/latest.json';
...
// How long Next.js may reuse a Finnhub response before refetching. This is
// the fix for a real incident: with zero caching, every single pageview
// triggered ~50-70 Finnhub calls, and the free tier's 60/min limit got hit
// after only light testing traffic, silently falling back to sample data.
// A short server-side revalidate window decouples "data freshness" from
// "number of visitors" — Finnhub gets called at most once per window,
// regardless of how many people load the page in that window. This does
// NOT reintroduce the earlier stale-data bug: that bug was a long-lived
// CDN Cache-Control header that kept serving a pre-redeploy snapshot for up
// to 30 minutes; this is a short, server-side, per-deployment cache with a
// 2-minute ceiling, invalidated automatically on every redeploy.
const FINNHUB_REVALIDATE_SECONDS = 120;
const AI_VERDICTS_REVALIDATE_SECONDS = 600; // this file only changes once/day in practice
...
async function fetchJson(url) {
  const res = await fetch(url, { next: { revalidate: FINNHUB_REVALIDATE_SECONDS } });
...
async function fetchAiVerdicts() {
  try {
    ...
    console.error('AI verdicts fetch failed (non-fatal):', err.message);
    return {};
  }
}
...
async function fetchLiveStocks(apiKey) {
  ...
  const [finalStocks, aiVerdicts] = await Promise.all([
    mapLimit(top, 8, async ({ ticker, quote, profile, exchange }) => {
      let headline = null;
      let url = null;
      let source = null;
      try {
  ...
  const merged = finalStocks.filter(Boolean).map((stock) => {
    const ai = aiVerdicts[stock.ticker];
    return {
      ...stock,
      aiRecommendation: ai?.recommendation || null,
      aiRiskLevel: ai?.riskLevel || null,
      aiEntryZone: ai?.entryZone || null,
      aiTargetPrice: ai?.targetPrice || null,
      aiStopLoss: ai?.stopLoss || null,
      aiFundamental: ai?.fundamental || null,
      aiFundamentalHE: ai?.fundamentalHE || null,
      aiTechnical: ai?.technical || null,
      aiTechnicalHE: ai?.technicalHE || null,
      aiRisk: ai?.risk || null,
      aiRiskHE: ai?.riskHE || null,
      aiRationale: ai?.rationale || null,
      aiRationaleHE: ai?.rationaleHE || null
    };
  });
  ...
  try {
    ...
    return Response.json(stocksData, {
      headers: {
        ...
        // The outer HTTP response itself is never cached long-lived (that
        // was the earlier bug). Freshness at the Finnhub-call level is now
        // controlled by FINNHUB_REVALIDATE_SECONDS above instead.
