export default function StockCard({ stock }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 hover:border-slate-600 transition-colors">
      ...
      {stock.isLive === true && (
        <div className="mt-1 text-[10px] text-gray-600 leading-snug">
          Live data via Finnhub{fetchedTime ? ` · fetched ${fetchedTime}` : ''}. 52-week range and volume are Finnhub free-tier basic financials (not intraday). Sentiment is a heuristic score, not a prediction — verify before acting.
        </div>
      )}
    </div>
  );
